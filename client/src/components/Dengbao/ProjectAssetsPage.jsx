...
function ProjectAssetsPage() {
  ...
  const getStatusClass = (status) => {
    return status === '在线' ? 'status-online' : 'status-offline';
  };
    ...
    <div className="project-page-container">
      ...
      <div className="stats-summary">
        ...
        <div className="stat-item">
          ...
          <span className="stat-value">{assets.filter(a => a.status === '在线').length}</span>
        </div>
        <div className="stat-item">
          <span className="stat-label">离线设备</span>
          <span className="stat-value">{assets.filter(a => a.status === '离线').length}</span>
        </div>
      </div>

      <div className="search-bar">
        <div className="search-input-wrapper">
          <input type="text" placeholder="搜索资产名称、IP地址..." className="search-input" />
